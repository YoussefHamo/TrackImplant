...
function rowToAppointment(row: Record<string, unknown>): Appointment {
  return {
    ...
    procedure_name: row.procedure_name as string | undefined,
  };
}
...
export const appointmentService = {
  async getAll(branchId?: string | null): Promise<Appointment[]> {
    let q = supabase
      .from('appointments')
      .select('*, patients!inner(full_name, phone), doctor:users!fk_appointments_doctor_id(full_name)')
  ...
  async getById(id: string): Promise<Appointment | null> {
    ...
    return {
      ...
      doctor_name: (data as any).doctor?.full_name,
      patient_phone: (data as any).patients?.phone,
    };
  },
  ...
  async create(appointment: Omit<Appointment, 'id'>, change_reason?: string, reason_category?: string): Promise<void> {
    const payload: Record<string, unknown> = {
      ...
      procedure_name: appointment.procedure_name || null,
